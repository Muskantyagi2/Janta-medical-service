...
// Removed Razorpay instance for mock payments
...
export const placeOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod == "online") {
      // Create mock Razorpay order instead of real one
      const mockRazorOrder = {
        id: `mock_order_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`,
        ...
        test_mode: true,
        notes: {
          test_payment: "This is a mock payment for testing",
        },
      };
      ...
      const newOrder = await Order.create({
        ...
        totalAmount,
        shopOrders,
        razorpayOrderId: mockRazorOrder.id,
        payment: false,
      });

      return res.status(200).json({
        razorOrder: mockRazorOrder,
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    // Simulate payment verification instead of real Razorpay call
    const payment = await simulatePayment(100); // Mock payment
    ...
    order.payment = true;
    order.razorpayPaymentId = razorpay_payment_id || payment.id;
    await order.save();
...
export const acceptOrder = async (req, res) => {
  try {
    ...
    // Populate order with user and delivery boy details
    await order.populate("user", "socketId");
    await order.populate(
      "shopOrders.assignedDeliveryBoy",
    ...
    if (io && order.user.socketId) {
      console.log(
        `Emitting update-status to user ${order.user._id} with socketId ${order.user.socketId}`
      );
      console.log(
        `Order ID: ${order._id}, Shop ID: ${shopOrder.shop}, Status: ${shopOrder.status}`
      );
      ...
      console.log("No IO instance or user socket ID not found");
      console.log("IO:", !!io, "User Socket ID:", order.user.socketId);
    }
...
export const sendDeliveryOtp = async (req, res) => {
  try {
    ...
    shopOrder.otpExpires = Date.now() + 10 * 60 * 1000; // Extended to 10 minutes
    await order.save();
